...
class ProductDetailsPage extends BasePage {
  ...
  async assertProductReviewCount(productName, productReviewCount) {
    await expect(
      this.page.locator(`//h1[text()="${productName}"]/following-sibling::div/p`).first(),
    ).toContainText(productReviewCount);
  }
  ...
  async assertProductPrice(productName, productPrice) {
    await expect(
      this.page
        .locator(
          `//h1[text()="${productName}"]/following-sibling::p[contains(@class, 'font-medium')]`,
        )
        .first(),
  ...
  async fillReviewForm() {
    ...
    await this.page.fill(
      this.locators.giveYourOpinionInput,
