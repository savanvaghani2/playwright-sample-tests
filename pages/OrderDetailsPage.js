...
class OrderDetailsPage extends BasePage {
  ...
  async assertOrderInformationTitle() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.orderInformationTitle),
    ).toBeVisible();
  }
  ...
  async assertOrderConfirmedTitle() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.orderConfirmedTitle),
    ).toBeVisible();
  }

  async assertOrderConfirmedMessage() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.orderConfirmedMessage),
    ).toBeVisible();
  }

  async assertShippingDetailsTitle() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.shippingDetailsTitle),
    ).toBeVisible();
  }

  async assertShippingEmailValue(email) {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.shippingEmailValue),
    ).toContainText(email);
  }

  async assertPaymentMethodAmount(amount) {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.paymentMethodAmount),
    ).toContainText(amount);
  }

  async assertDeliveryAddressLabel() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.deliveryAddressLabel),
    ).toBeVisible();
  }

  async assertDeliveryAddressValue() {
    await expect(
      this.page.getByTestId(this.locators.orderInformation.deliveryAddressValue),
    ).toBeVisible();
  }
  ...
  async assertOrderSummaryProductName(productName) {
    await expect(
      this.page.getByTestId(this.locators.orderSummary.orderSummaryProductName),
    ).toContainText(productName);
  }

  async assertOrderSummaryProductQuantity(quantity) {
    await expect(
      this.page.getByTestId(this.locators.orderSummary.orderSummaryProductQuantity),
    ).toContainText(quantity);
  }

  async assertOrderSummaryProductPrice(price) {
    await expect(
      this.page.getByTestId(this.locators.orderSummary.orderSummaryProductPrice),
    ).toContainText(price);
  }

  async assertOrderSummarySubtotalValue(subtotal) {
    await expect(this.page.getByTestId(this.locators.orderSummary.subtotalValue)).toContainText(
      subtotal,
    );
  }

  async assertOrderSummaryShippingValue(shipping) {
    await expect(this.page.getByTestId(this.locators.orderSummary.shippingValue)).toContainText(
      shipping,
    );
  }
