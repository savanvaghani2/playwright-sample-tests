...
class OrderPage extends BasePage {
  ...
  async verifyOrderSummary(productName, quantity, amount, status) {
    ...
    await expect(this.page.locator(this.locators.orderItemQuantity)).toContainText(
      `Qty: ${quantity}`,
    );
  ...
  async clickCancelOrderButton(buttonIndex = 2) {
    await this.page
      .locator(`(//button[normalize-space()='Cancel'])[${buttonIndex}]`)
      .click({ force: true });
  }
  ...
  async verifyCancellationConfirmationMessage() {
    ...
    await expect(this.page.locator(this.locators.toasterMessage)).toContainText(
  ...
  async verifyOrderStatusIsCanceled(productName) {
    await expect(
      this.page
        .locator(`//h3[normalize-space()='${productName}']`)
        .locator('xpath=./ancestor::div[contains(@class, "card")]//div[contains(@class, "badge")]'),
    ).toHaveText('Canceled');
  }

  async clickOnPaginationButton(pageNumber) {
    await this.page
      .locator(this.locators.paginationButton.replace('{}', pageNumber))
      .click({ force: true });
  }

  async verifyProductInOrderList(productName) {
    await expect(
      this.page.locator(this.locators.productNameInOrderList.replace('{}', productName)),
    ).toBeVisible();
  }

  async verifyPriceAndQuantityInOrderList(priceAndQuantity) {
    await expect(
      this.page.locator(this.locators.priceAndQuantityInOrderList.replace('{}', priceAndQuantity)),
    ).toBeVisible();
  }
